...
export default {
  ...
  beforeMount: function() {
    ...
    window.addEventListener('popstate', () => { this.$store.commit('setOpCardShow', {flag: false}) })
  },
  methods: {
    deleteUser: async function(row) {
      const res = await this.$store.dispatch('user/deleteUser', row)
    ...
    navigateTo: function(flag, row) {
      if (flag === 'add') {
        this.$router.push({path: '/user/add'})
      } else if (flag === 'modify') {
        this.$router.push({path: '/user/modify', name: '/user/modify', params: row})
      } else {
        this.$router.push({path: '/user/details', name: '/user/details', params: row})
      }
      setTimeout(() => {
        this.$store.commit('setOpCardShow', {flag: true})
      }, 0)
    },
  ...
  unmounted: function() {
    window.removeEventListener('popstate', () => this.$store.commit('setOpCardShow', {flag: false}))
  }
}
</script>
